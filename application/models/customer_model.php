...
class Customer_model extends CI_Model {
    ...
    function __construct() {
 	...
 	public function addCustomer($data) {
 		...
 		$result = $this->db->query( "INSERT INTO customer SET customer_name = " . $this->db->escape($data['customer_name']) . " , customer_surname = " . $this->db->escape($data['customer_surname']) . " ,  customer_email = " . $this->db->escape($data['customer_email']) . " , customer_company = " . $this->db->escape($data['customer_company']) . " , customer_company_image = " . $this->db->escape($data['customer_company_image']) . " ,customer_phone = " . $this->db->escape($data['customer_phone']) . " ,  customer_address = " . $this->db->escape($data['customer_address']) . "  , customer_tax_office = " . $this->db->escape($data['customer_tax_office']) . " , customer_tax_no = " . $this->db->escape($data['customer_tax_no']) . "  ,customer_date_added = now() , customer_status = '" . (int)($data['customer_status']) . "'  ");
        $customer_id = $this->db->insert_id();
        $this->db->query("INSERT INTO feed SET type='6' , dataID='". $customer_id ."' , added_date = now() ");
 		return $result;
 	}
    ...
    public function addCustomerAjax($data){
        ...
        $result = $this->db->query( "INSERT INTO customer SET customer_name = " . $this->db->escape($data[0]) . " , customer_surname = " . $this->db->escape($data[1]) . " ,  customer_email = " . $this->db->escape($data[2]) . " , customer_company = " . $this->db->escape($data[3]) . " ");
        $customer_id = $this->db->insert_id();
        $this->db->query("INSERT INTO feed SET type='6' , dataID='". $customer_id ."' , added_date = now() ");
        return $customer_id;
    }
 	...
 	public function updateCustomer($data , $customer_id){
        $this->db->query("INSERT INTO feed SET type='7' , dataID='". $customer_id ."' , added_date = now() ");
 		$result = $this->db->query("UPDATE customer SET customer_name = " . $this->db->escape($data['customer_name']) . " , customer_surname = " . $this->db->escape($data['customer_surname']) . " , customer_email = " . $this->db->escape($data['customer_email']).  " , customer_company = " . $this->db->escape($data['customer_company']) . " , customer_company_image = " . $this->db->escape($data['customer_company_image']) . " ,customer_phone = " . $this->db->escape($data['customer_phone']) . " , customer_address = " . $this->db->escape($data['customer_address']) . " , customer_tax_office = " . $this->db->escape($data['customer_tax_office']) . " , customer_tax_no = " . $this->db->escape($data['customer_tax_no']) . "  ,customer_date_updated = now() , customer_status = '" . (int)($data['customer_status']) . "' WHERE customer_id = '" . (int)$customer_id . "' ");
 		return $result;
 	}
