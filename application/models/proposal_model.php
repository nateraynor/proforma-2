...
class Proposal_model extends CI_Model {
    ...
    function __construct() {
 	...
 	public function getProposals($filters) {
 		$result = $this->db->query("SELECT * FROM proposal p LEFT JOIN proposal_to_customer ptc ON p.proposal_id = ptc.proposal_id LEFT JOIN customer c ON ptc.customer_id = c.customer_id");
	...
	public function updateProposal($data,$proposal_id) {
		...
		$this->db->query("UPDATE proposal_note SET proposal_id = '" . $proposal_id . "', proposal_note_content = " . $this->db->escape($proposal_note_content) . ", proposal_note_user = '" . $this->session->userdata['user_id'] . "', proposal_note_status = '1', proposal_note_date_added = now(), proposal_note_date_updated = now() WHERE proposal_id = '" . (int)$proposal_id . "'");

		$this->db->query("DELETE FROM proposal_product WHERE proposal_id = '" . (int)$proposal_id . "' ");


		foreach ($data['proposal_product'] as $product) {
           $result = $this->db->query("INSERT INTO proposal_product SET product_id = '" . (int)$product['product_id'] . "' ,proposal_id = '" . (int)$proposal_id . "' , product_quantity = " . $this->db->escape($product['product_quantity']) . ", product_price = " . $this->db->escape($product['product_price']) ." , product_discount = " . $this->db->escape($product['product_discount']) ." , product_discount_type = " . (int)$product['product_discount_type'] ." , product_tax_rate = " . $this->db->escape($product['product_tax_rate']) ."");
        }

		foreach($data['proposal_customers'] as $customer_id) {
			$this->db->query("UPDATE proposal_to_customer SET proposal_id = '" . (int)$proposal_id . "', customer_id = '" . $customer_id . "' WHERE proposal_id = '" . (int)$proposal_id . "'");
		}
